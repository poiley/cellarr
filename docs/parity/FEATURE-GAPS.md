...
| **3 — Library management** | Delete movie/series (+files) + recycle bin; manual import (scan→match→import loose files); per-season/episode monitoring options; wire bulk delete | **3a delete/recycle/bulk-delete ✅** (DELETE routes + crash-safe recycle bin w/ path-escape guard + UI confirm); 3b manual-import + monitoring ☐ |
